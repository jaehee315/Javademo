...
/*
 * 다형성(polymorphism)
 * 1.사전적 의미는 "여러가지 형태를 가질 수 잇는 능력"을 의미
 * 2. 자바에서 한 타입의 참조변수로 여러 타입의 객체를 참조하는 기술이다.
 * => 부모클래스가 동일한 메시지로 자식 클래스들을 서로 다르게 동작시키는 객체 지향 원리이다.
 * => 하나의 타입으로 상속관계에 있는ㄴ 여러 객체을 다루는 기술이다.
 * 
 * 바인딩(binding) : 메소드 호출을 실제 메소드의 몸체와 연결하는 기술이다.
 * 1. 정적 바인딩(Static binding) : 컴파일 단계에서 어떤 클래스의 메소드가 호출된는지 연결하는 기술
 * 2. 동적 바인딩(dynamic binding) : 실행단계에서 어떤 클래스의 메소드가 호출된는지 연결하는 기술
 */
